...
WHERE tc1.curso_academico = 2021 AND tc1.id_titulacion = t1.id AND tc1.id_centro = c1.id AND c1.id_localidad = l1.id 
AND t1.id IN (SELECT t2.id
			  FROM titulacion_por_centro tc2, titulacion t2, centro c2
		      WHERE tc2.curso_academico = 2021 AND tc2.id_titulacion = t2.id AND tc2.id_centro = c2.id 
                    AND c2.id_localidad = l1.id
			  ORDER BY tc2.indice_ocupacion DESC LIMIT 2)
ORDER BY l1.nombre;