...
WHERE am.id_centro = c.id AND am.id_universidad = u.id AND am.in_out = 'OUT' AND am.curso_academico = 2021 AND
      am.plazas_asignadas_out = (SELECT MAX(am2.plazas_asignadas_out)
							     FROM acuerdo_movilidad am2
							     WHERE am2.in_out = 'OUT' AND am2.curso_academico = 2021 AND am2.id_centro = am.id_centro)
ORDER BY c.nombre;