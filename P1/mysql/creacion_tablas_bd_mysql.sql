...
UPDATE titulacion_por_centro tc,
    (SELECT c.id as id_centro, t.id as id_titulacion, r.curso_academico as anyo, r.alumnos_graduados as graduados, r.alumnos_con_reconocimiento as reconocimiento, 
    ...
    r.creditos_reconocidos as creditos_reconocidos, r.duracion_media_graduados as duracion, r.tasa_exito as exito
    FROM resultados_titulaciones r, centro c, titulacion t
    WHERE tipo_estudio = 'Grado' AND SUBSTRING(r.estudio, 3) = t.nombre AND c.nombre = r.centro) AS t1
SET tc.alumnos_graduados = t1.graduados, tc.alumnos_con_reconocimiento = t1.reconocimiento, tc.alumnos_movilidad_entrada = t1.alumnos_entrada, 
tc.alumnos_movilidad_salida = t1.alumnos_salida, tc.creditos_matriculados = t1.creditos_matriculados, tc.creditos_reconocidos = t1.creditos_reconocidos,
duracion_media_graduados = t1.duracion, tasa_exito = t1.exito
WHERE tc.id_centro = t1.id_centro AND tc.id_titulacion = t1.id_titulacion AND tc.curso_academico = t1.anyo;

/* Insertar datos de la tercera tabla temporal en la tabla titulacion_por_centro */
UPDATE titulacion_por_centro tc,
    (SELECT c.id as id_centro, t.id as id_titulacion, n.curso_academico as anyo, n.nota_corte_definitiva_julio as nota_julio, n.nota_corte_definitiva_septiembre as nota_sep
    FROM notas_corte n, centro c, titulacion t
    WHERE n.centro = c.nombre AND t.nombre = SUBSTRING(n.estudio, 8)) as t1
SET tc.nota_corte_julio = t1.nota_julio, tc.nota_corte_septiembre = t1.nota_sep
WHERE tc.id_centro = t1.id_centro AND tc.id_titulacion = t1.id_titulacion AND tc.curso_academico = t1.anyo;

/* Insertar nº de abandonos voluntarios en la tabla titulacion_por_centro */
UPDATE titulacion_por_centro tc,
    (SELECT a.curso_academico as curso, c.id as id_centro, t.id as id_titulacion, SUM(a.alumnos_interrumpen_estudios) as abandonos
...
SET tc.abandonos_voluntarios = t1.abandonos
